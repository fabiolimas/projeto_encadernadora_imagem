<?php
		...
		$busca_lojas="SELECT e.loja_id, l.loja_nome, e.os_alb, e.data_envio_loja, e.data_recebe_lab, e.data_recebe_enca, e.status FROM  entrega_alb as e join lojas as l on l.loja_id=e.loja_id and  os_alb='$os' order by e.data_envio_loja desc";
...
<body>
	...
	<form name="form" action="processa_envio.php" method="POST">
		
		<?php
			
	
				echo "<input type='hidden' value='$id_loja' name='idloja'>
		Loja: <input type='text' name='n_loja' value='$lojan ' readonly size='10'>
		...
		Entrada Lab: <input type='text' name='datalab' value='$datalab'size='8' readonly>
			Entrada ENC: <input type='text' name='dataenc' value='$data_receb_enc'size='8' readonly>
		Status da Os: <input type='text' value='$statuss' readonly size='8'>
			<hr>				
		<br><br>
		Confirma Envio: <button id='bt'><img src='imagens/visto.png' width=20px></button>
		";?>
		<br><br>
	...
	<a href="recebidos_enc.php"><button id="btn">Voltar</button></a>
</div>
</body>
</html>