...
<?php
	...
	$atualizar="UPDATE entrega_alb set 	data_recebe_lab='$data', status_receb='$status', observacao='$observacao' where os_alb='$os'";
...
$lista_recebimento="SELECT l.loja_nome, e.os_alb, e.data_envio_loja, e.status, e.data_recebe_enca, e.status_receb, e.data_recebe_lab,e.observacao FROM entrega_alb as e join lojas as l on l.loja_id=e.loja_id order by e.data_envio_loja desc"; 
...
while($mostra_recebidos=mysqli_fetch_array($query_recebimento)){
	...
	$observac=$mostra_recebidos['observacao'];
...
?>
	...
	<button id="btn" onclick="history.go(-2)">Voltar</button><br><br>
